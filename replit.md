...
- **Add Outfit** (`/add-outfit`): Photo capture/upload with cropping, then saves outfit and navigates to tag page. Also supports a Paste flow: reads transparent PNGs from clipboard (e.g. Apple "Copy Subject"), shows a background picker with 6 editorial Depop-style gradients (Chalk, Sand Drift, Lilac Mist, Peach Haze, Slate Deep, Sage Blur) rendered via Canvas at 900×1200px, live 3:4 preview; opaque pasted images enter normal crop flow; Paste button hidden on unsupported browsers.
